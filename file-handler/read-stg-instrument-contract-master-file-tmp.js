...
export default function readStgInstrumentContractMasterFile() {
    return new Promise((resolve, reject) => {
        ...
        try {
            csv.parseFile(`${process.cwd()}/data/${FILE_NAME.STG_INSTRUMENT_CONTRACT_MASTER}`)
                .on('error', (error) => {
                    console.log(`readStgInstrumentContractMasterFile - error: ${error.message}`);
                    reject(error);
                })
                .on('data', (row) => {
                    if (row?.length > 0) {
                        if (row.length !== CSV_HEADER.length) {
                            console.error(`readStgInstrumentContractMasterFile - invalid number of elements per line`);
                            return;
                        }
                        line++;
                        if (line === 1) {
                            if (!checkHeaderFormat(row)) {
                                console.error(`readStgInstrumentContractMasterFile - invalid header`);
            ...
            console.error(`readStgInstrumentContractMasterFile - catch error: ${error.message}`);
            reject(error);
        }
    });
}
