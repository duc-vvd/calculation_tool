...
export default class FileHandler {
  ...
  readFile() {
    return new Promise((resolve, reject) => {
      ...
      try {
        csv
          .parseFile(`${process.cwd()}/data/${FILE_NAME[this.file]}`)
          .on("error", (error) => {
            console.log(
              `File Handler - ${this.file} - error: ${error.message}`
            );
            reject(error);
          })
          .on("data", (row) => {
            if (row?.length > 0) {
              if (row.length !== this.csvHeader.length) {
                console.error(
                  `File Handler - ${this.file} - invalid number of elements per line`
                );
                return;
              }
              line++;
        ...
        console.error(
          `File Handler - ${this.file} - catch error: ${error.message}`
        );
        reject(error);
      }
    });
